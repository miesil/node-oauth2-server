...
function usePasswordGrant (done) {
  ...
  var self = this;
  return this.model.getUser(uname, pword, this.req, this.res, function (err, user) {
    if (err && !user) return done(error('server_error', false, err));
    if (err && user)  return done(error('invalid_grant', err.message, err));
    if (!user) {
